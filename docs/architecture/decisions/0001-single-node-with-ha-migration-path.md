...
| Thành phần       | Chi tiết                                                |
| ---------------- | ------------------------------------------------------- |
| Host vật lý      | Intel Xeon E5-2680v4, 32 GB RAM — cài Proxmox VE        |
| VM ecoma (K3s)   | 24 vCPU, 48 GB RAM (overcommit với ZSwap zstd)          |
| Storage NVMe     | ~430 GB (thin LVM từ 2×476.9 GB NVMe pool)              |
| Storage HDD      | ~11 TB (thin LVM từ 2×5.5 TB HDD pool)                 |
| Tailscale        | Cài trên Proxmox host — quản lý qua private mesh VPN    |
| Backup Server    | j1900 8 GB RAM, Proxmox Backup Server, 2×6 TB HDD       |

K3s cluster chạy bên trong VM trên Proxmox — không phải trực tiếp trên bare metal. Điều này mang lại linh hoạt (VM snapshot, migrate disk, mở rộng storage qua Proxmox) nhưng thêm một lớp SPOF.
...
Triển khai toàn bộ hệ thống trên **một máy chủ vật lý duy nhất** cài **Proxmox VE**, với K3s chạy bên trong một VM single-node (SQLite datastore), sử dụng **OpenEBS LVM-LocalPV** cho persistent storage. ArgoCD quản lý toàn bộ qua GitOps pattern.

**Các quyết định đi kèm:**

| Thành phần        | Quyết định hiện tại                                            | Lý do                                              |
| ----------------- | -------------------------------------------------------------- | -------------------------------------------------- |
| Hypervisor        | Proxmox VE trên bare metal                                     | VM snapshot, dễ migrate, thin LVM linh hoạt        |
| RAM VM            | 48 GB overcommit (host vật lý 32 GB + ZSwap zstd)             | Đủ cho toàn bộ workload giai đoạn đầu              |
...
- **Proxmox VM**: VM snapshot + migrate disk giúp chuyển K3s VM sang host mới mà không cần rebuild từ đầu.
...
### Vertical Scaling trước HA

Trước khi migration sang HA, hệ thống có thể scale dọc trong giới hạn phần cứng vật lý:

| Nhu cầu   | Hành động                                               | Kết quả                                         |
| --------- | ------------------------------------------------------- | ----------------------------------------------- |
| Thiếu RAM | Thêm 64 GB RAM vật lý (32 GB → 96 GB)                   | VM có thể nâng lên ~144 GB với ZSwap zstd       |
| Thiếu CPU | Thay CPU 2680v4 → 2697v4 (14c→18c)                      | Tăng vCPU cấp cho VM                            |
| Thiếu NVMe| Mở rộng VM disk qua Proxmox → resize vg-nvme             | Mở rộng không downtime K8s workload             |

> **Giai đoạn kinh doanh thực tế**: Upgrade lên máy chủ vật lý mạnh hơn nhưng vẫn giữ Proxmox để migrate VM liền mạch — không cần rebuild cluster từ đầu.
